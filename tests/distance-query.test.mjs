...
test('TASK-041 returns semantic identity and Length for an explicitly targeted representation', () => {
...
test('TASK-041 distance is zero inside or on the represented solid region', () => {
...
test('TASK-041 cylinder cap and box distances use the representation geometry', () => {
  assert.equal(
    toMillimetres(
  ...
  assert.equal(
    toMillimetres(
      query.execute(p(0, 0, -6), structureId('structure.fixture.skin')).distance,
    ),
    5,
  );
});

test('TASK-041 semantic target is explicit rather than inferred from proximity or names', () => {
  ...
  assert.notEqual(toMillimetres(vein.distance), toMillimetres(artery.distance));
...
test('TASK-041 rejects unknown targets, duplicate bindings, invalid points, and invalid adapter distances', () => {
  assert.throws(
    () => query.execute(p(0, 0, 0), structureId('structure.fixture.missing')),
    DistanceQueryFailure,
  );
  assert.throws(() => new DistanceQuery([entries[0], entries[0]]), /Duplicate/);
  assert.throws(
